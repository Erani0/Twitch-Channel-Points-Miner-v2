...
class MakePredictionsTest(unittest.TestCase):
    ...
    def test_make_predictions_handles_null_make_prediction_payload(self):
        ...
        with patch.object(Twitch, "post_gql_request", return_value=response) as mocked_post:
            with patch("TwitchChannelPointsMiner.classes.Twitch.logger.error") as mocked_error:
    ...
    def test_make_predictions_uses_gql_error_logger_before_payload_check(self):
        ...
        with patch.object(Twitch, "post_gql_request", return_value=response):
            with patch("TwitchChannelPointsMiner.classes.Twitch.logger.warning") as mocked_warning:
                with patch("TwitchChannelPointsMiner.classes.Twitch.logger.error") as mocked_error:
                    self.twitch.make_predictions(self._event())
